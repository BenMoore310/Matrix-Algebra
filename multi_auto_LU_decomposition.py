...
for key, value in time_dict.items():
    # value = value.squeeze()
    # print(key,value)
    # print(value, size_values)
    plt.plot(size_values, value, alpha = 0.1, label=('Run', (key+1)))
...
coeffs, coeffs_covarience = curve_fit(test_function, size_values, average)
fit = coeffs[0]*(size_values**coeffs[1])
# print(coeffs)
plt.plot(size_values, average, alpha = 0.6, label = 'Averaged data')
plt.plot(size_values, fit, label = 'Curve fit ax^b, power = {}'.format(coeffs[1]), alpha = 1)
